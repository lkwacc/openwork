...
export default function SessionView(props: SessionViewProps) {
  ...
  const handleOpenBrowserQuickstart = async () => {
    ...
    try {
      ...
      const commandPrompt = t("session.quickstart.open_browser_chat_prompt_with_url", currentLocale()).replace("{url}", target);
          ...
          <header class="z-10 flex h-12 shrink-0 items-center justify-between border-b border-dls-border bg-dls-surface px-4 md:px-6">
            ...
            <div class="flex items-center gap-1.5 text-gray-10">
              <div class="hidden items-center gap-1 sm:flex">
                <button
                  type="button"
                  class="h-9 rounded-md px-2.5 text-[11px] font-mono text-gray-10 transition-colors hover:bg-gray-2/70 hover:text-dls-text"
                    ...
                    <div class="text-center py-16 px-6 space-y-6">
                      ...
                      <div class="space-y-2">
                        <h3 class="text-xl font-medium">
                          {t("session.empty_title", currentLocale())}
                        </h3>
                        <p class="text-dls-secondary text-sm max-w-sm mx-auto">
                          快捷按钮已移动到顶部，或直接在下方输入你的需求。
                        </p>
                      </div>
